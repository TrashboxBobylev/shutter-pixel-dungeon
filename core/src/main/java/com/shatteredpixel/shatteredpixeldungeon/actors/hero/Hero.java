...
import com.shatteredpixel.shatteredpixeldungeon.items.scrolls.ScrollOfDebug;
...
public class Hero extends Char {
	...
	public Hero() {
		...
		// Note DeviceCompat.isDebug prevents scroll of debug from appearing in release versions of the game.
	...
	public boolean shoot( Char enemy, MissileWeapon wep ) {
		...
		if (hit && (heroClass == HeroClass.DUELIST || heroClass == HeroClass.BEST_CHARACTER) && wasEnemy){
	...
	public float attackDelay() {
		...
		if (!RingOfForce.fightingUnarmed(this)) {
			...
			return delay * belongings.attackingWeapon().delayFactor( this )*del;
			...
			if (wep instanceof MissileWeapon && !(wep instanceof SpiritBow.SpiritArrow) && enemy != this) {
				Actor.add(new Actor() {
					...
					protected boolean act() {
						if (enemy.isAlive() && Dungeon.gold >= SpiritBow.SNIPER_SPECIAL_COST) {
	...
	public boolean handle( int cell ) {
		...
		if (Dungeon.level.map[cell] == Terrain.ALCHEMY && cell != pos) {
			...
			if ((ch.alignment != Alignment.ENEMY || ch instanceof NPC) && ch.buff(Amok.class) == null) {
	...
	public static int maxExp( int lvl ){
		return (int) Math.round((5 + lvl * 5) * Math.pow(1.25f, Math.max(lvl - 30, 0)));
	}
	...
	public boolean search( boolean intentional ) {
		...
		int distance = (heroClass == HeroClass.ROGUE || heroClass == HeroClass.BEST_CHARACTER) ? 2 : 1;
